...
class NaveEspacial(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.ImagenExplosion = pygame.image.load('Imagenes/explosion.png')      #Cargamos la imagen de la explosion
        self.ImagenExplosion = pygame.transform.scale(self.ImagenExplosion, (92, 84))    #Reescalamos la imagen de la explosion
        ...
        self.listaProyectiles = []      #En esta lista se almacenaran los objetos disparos que tendran un limite para la nave
        self.sonidoDisparo = pygame.mixer.Sound('Sonidos/Shoot.wav')    #Almacena el sonido de cada disparo de la nave
    ...
    #Este metodo hace que la nave pase de un extremo a otro de la pantalla si es que sale de esta
    def movimiento(self):
        #Mientras siga con vida la nave se podra mover
        if self.vida == True :
    ...
    #Este metodo realiza el movimiento lateral de la nave
    def movimientoLateral(self):
        self.rect.centerx += self.velocidad     #La nave se movera lateralmente dependiendo de su velocidad
        self.movimiento()                       #Verificamos si el movimiento realizado pasa los lados de la pantalla
    #Este metodo reproduce el sonido de disparo de la nave
    def sonidos(self):
        self.sonidoDisparo.play()
    #Este metodo maneja los disparos realizados por la nave
    def disparar(self, x, y):
        miProyectil = Proyectil(x, y,'Imagenes/disparo1.png', True)     #Cada vez que se llama el metodo se crea un objeto proyectil
        self.listaProyectiles.append(miProyectil)       #Cada proyectil creado se agrega a ña lista de la nave
    #Este metodo dibua la nave en su posicion actual   
    def dibujar(self, superficie):
        superficie.blit(self.Nave, self.rect)   #Superpone la imagen en el fondo
    #Este metodo se encarga de destruir la nave al perder
    def destruccion(self):
        self.vida = False
        self.velocidad = 0
        self.Nave = self.ImagenExplosion
...
class Proyectil(pygame.sprite.Sprite):
    ...
    def __init__(self, posx, posy, ruta, personaje):
        #Atributos
        pygame.sprite.Sprite.__init__(self)     #Inicializacmos el pygame
        self.personaje = personaje  #Almacena el personaje que realiza el disparo (True=nave, False=enemigo)
        self.ImagenProyectil = pygame.image.load(ruta)  #Carga la imagen del proyectil usando la ruta dada
        if self.personaje == True:   #Sirve para diferenciar si el proyectil que se debe de crear es de la nave o un enemigo
            self.ImagenProyectil = pygame.transform.scale(self.ImagenProyectil, (21, 21))   #Reescalamos el tamaño del proyectil de la nave
        else:
            self.ImagenProyectil = pygame.transform.scale(self.ImagenProyectil, (20, 34))   #Reescalamos el tamaño del proyectil del enemigo
        self.rect = self.ImagenProyectil.get_rect() #Almacena la ubicacion del proyectil
        self.rect.top = posy    #Ubicamos la parte superior del proyectil
        self.rect.centerx = posx    #Ubicamos el centro del proyectil  
        if self.personaje == True:
            self.velocidadDisparo = 7
        else:
            self.velocidadDisparo = 5
    #Este metodo define la trayectoria del disparo dependiendo del personaje    
    def trayectoria(self):
        if self.personaje == True:
            self.rect.top = self.rect.top - self.velocidadDisparo   #Si el disparo lo realiza la nave, entonces sube
        else:
            self.rect.top = self.rect.top + self.velocidadDisparo   #Si el disparo lo realiza el enemigo, entonces baja
    #Este metodo dibuja el proyectil en su posicion actual  
    def dibujar(self, superficie):
        superficie.blit(self.ImagenProyectil, self.rect)    #Superpone la imagen en el fondo
    #Este metodo regresa la puntuacion si el proyectil colisiona con alguno de los enemigos
    def colisionEnemigo(self, listProyectiles, listEnemigos, proyectil, score, puntos):
        for enemigo in listEnemigos:    #Por cada enemigo con vida
            if self.rect.colliderect(enemigo.rect):     #Verificamos si el proyectil colisiona con el enemigo
                enemigo.destruccionEnemigo()    #Animacion de destruccion del enemigo
                listEnemigos.remove(enemigo)    #Removemos el enemigo de la lista de enemigos
                listProyectiles.remove(proyectil)  #Removemos el proyectil que impacto
                score += puntos     #Sumamos puntos por el enemigo destruido
        return score

class Invasor(pygame.sprite.Sprite):
    
    def __init__(self, posx, posy, distancia, ruta_imagen, ancho, alto):
        pygame.sprite.Sprite.__init__(self)
        self.ImagenInvasor = pygame.image.load(ruta_imagen)
